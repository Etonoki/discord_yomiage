...
class ManageRole(commands.Cog):
    ...
    async def on_raw_reaction_add(self,payload):
        if payload.message_id == 667352897654095893:
            ...
            prof = guild.get_channel(638739366964887553)
            await member.send(f"いらっしゃいませ～\n{guild.name}のBOT、{self.bot.user.name}です\nよろしくお願いします♪")
            await asyncio.sleep(5)
            await member.send(f"自己紹介のテンプレートです！\n自由に追加したり削ったりしてくださいね\n{prof.mention}\n")
            await member.send(f"【名前】{member.name}\n【性別】\n【年齢】\n【趣味】\n【コアタイム】\n【一言】")
